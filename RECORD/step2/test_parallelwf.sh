...
sim_file=log_parallelwf_sim
ml_file=log_parallelwf_ml
al_file=log_parallelwf_al
...
start1=$(date +%s.%N)
mpiexec -n ${NTOTRANKS_SIM} --ppn ${NRANKS_PER_NODE_SIM} --env OMP_NUM_THREADS=1 --depth=1 --cpu-bind depth ./echo_and_sleep.sh 10 >> $sim_file
echo "Execution command is mpiexec -n ${NTOTRANKS_SIM} --ppn ${NRANKS_PER_NODE_SIM} --env OMP_NUM_THREADS=1 --depth=1 --cpu-bind depth ${CMD_SIM} >> $sim_file" >> $sim_file
end1=$(date +%s.%N)
...
do
	...
	(	
	start1=$(date +%s.%N); \
	mpiexec -n ${NTOTRANKS_SIM} --ppn ${NRANKS_PER_NODE_SIM} --env OMP_NUM_THREADS=1 --cpu-bind list:2:3:4:5:6:7:10:11:12:13:14:15:18:19:20:21:22:23:26:27:28:29:30:31 ./echo_and_sleep.sh 20 >> $sim_file; \
	echo "Command to run is mpiexec -n ${NTOTRANKS_SIM} --ppn ${NRANKS_PER_NODE_SIM} --env OMP_NUM_THREADS=1 --cpu-bind list:2:3:4:5:6:7:10:11:12:13:14:15:18:19:20:21:22:23:26:27:28:29:30:31 ${CMD_SIM} >> $sim_file" >> $sim_file; \
	end1=$(date +%s.%N); \	
	runtime1=$( echo "$end1 - $start1" | bc -l ); \
	echo "Time logging: running time for sim in seconds = ${runtime1}" >> $sim_file
	)&
	...
	(
	start2=$(date +%s.%N); \
	mpiexec -n ${NTOTRANKS_ML} --ppn ${NRANKS_PER_NODE_ML} --env OMP_NUM_THREADS=2 --cpu-bind list:0,1:8,9:16,17:24,25 ./set_affinity_gpu_polaris.sh ./echo_and_sleep.sh 20 >> $ml_file; \
	echo "Command to run is mpiexec -n ${NTOTRANKS_ML} --ppn ${NRANKS_PER_NODE_ML} --env OMP_NUM_THREADS=2 --cpu-bind list:0,1:8,9:16,17:24,25 ./set_affinity_gpu_polaris.sh $CMD_ML >> $ml_file" >> $ml_file; \
	end2=$(date +%s.%N); \
	runtime2=$( echo "$end2 - $start2" | bc -l ); \
	echo "Time logging: running time for ML in seconds = ${runtime2}" >> $ml_file; \
	if [[ $phase_idx != $(($num_phase - 1)) ]]; then start3=$(date +%s.%N); mpiexec -n 1 --ppn 1 --cpu-bind list:0 ./echo_and_sleep.sh 5 >> $al_file; echo "Command to run is mpiexec -n 1 --ppn 1 --cpu-bind list:0 sleep $active_learning_sleep_time_in_seconds" >> $al_file; end3=$(date +%s.%N); runtime3=$( echo "$end3 - $start3" | bc -l ); echo "Time logging: running time for active learning in seconds = ${runtime3}" >> $al_file; fi
	)&
	wait
done
...
start2=$(date +%s.%N)
mpiexec -n ${NTOTRANKS_ML} --ppn ${NRANKS_PER_NODE_ML} --depth=${NDEPTH} --cpu-bind depth --env OMP_NUM_THREADS=8 ./set_affinity_gpu_polaris.sh ./echo_and_sleep.sh 10 >> $ml_file
echo "Command to run is mpiexec -n ${NTOTRANKS_ML} --ppn ${NRANKS_PER_NODE_ML} --depth=${NDEPTH} --cpu-bind depth --env OMP_NUM_THREADS=8 ./set_affinity_gpu_polaris.sh $CMD_ML >> $ml_file" >> $ml_file
end2=$(date +%s.%N)
runtime2=$( echo "$end2 - $start2" | bc -l )
echo "Time logging: running time for ML in seconds = ${runtime2}" >> $ml_file
